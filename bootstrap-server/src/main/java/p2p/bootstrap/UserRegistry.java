...
public class UserRegistry {
    ...
    public void addUser(User user) {
        users.put(user.getUserId(), new UserEntry(user, Instant.now()));
        System.out.println("[Registry] User registered: " + user.getUsername());
    }
    ...
    public void removeUser(String userId) {
        ...
        if (entry != null) {
            System.out.println("[Registry] User unregistered: " + entry.user.getUsername());
        }
    }
    ...
    public List<User> searchByIp(String ip) {
        return users.values().stream()
            .map(entry -> entry.user)
            .filter(user -> user.getIpAddress().contains(ip))
            .collect(Collectors.toList());
    }
        ...
        while (running) {
            try {
                ...
                staleUserIds.forEach(userId -> {
                    UserEntry entry = users.remove(userId);
                    if (entry != null) {
                        System.out.println("[Registry] Removed stale user: " + entry.user.getUsername());
                    }
                });
